 ...
 * 
 */
Arena::Arena(Arena &&other) noexcept : blocks_(std::move(other.blocks_)), block_size_(other.block_size_) {
    other.blocks_.clear(); /** Blocks in other Arena have been moved over, clear references to them */
}

/**
 * @brief Move blocks from other Arena to this ie. this Arena already exists (different from move constructor)
 * 
...
void *Arena::allocate(size_t size, size_t alignment) {
    ...
    if (blocks_.empty()) {
        blocks_.push_back({
            reinterpret_cast<uint8_t *>(std::malloc(block_size_)),
            block_size_,
            0
        });
    ...
    if (block->used + total > block->size) {
        ...
        blocks_.push_back({
            reinterpret_cast<uint8_t *>(std::malloc(new_block_size)),
            new_block_size,
            0
        });
